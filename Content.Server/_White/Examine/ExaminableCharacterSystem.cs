...
namespace Content.Server._White.Examine
{
    public sealed class ExaminableCharacterSystem : EntitySystem
    {
        ...
        private void HandleExamine(EntityUid uid, ExaminableCharacterComponent comp, ExaminedEvent args)
        {
            ...
            var showExamine = _netConfigManager.GetClientCVar(actorComponent.PlayerSession.Channel, WhiteCVars.DetailedExamine);
            ...
            foreach (var slotEntry in slotLabels)
            {
                ...
                //&& !HasComp<StripMenuHiddenComponent>(slotEntity)) //carmine edit: we don't have this component and we kinda dont care
                {
            ...
            if (showExamine && _netConfigManager.GetClientCVar(actorComponent.PlayerSession.Channel, WhiteCVars.LogInChat))
        ...
        private void HandleExamine(EntityUid uid, MetaDataComponent metaData, ExamineCompletedEvent args)
        {
            ...
            if (TryComp<ActorComponent>(args.Examiner, out var actorComponent)
                && _netConfigManager.GetClientCVar(actorComponent.PlayerSession.Channel, WhiteCVars.DetailedExamine)
                && _netConfigManager.GetClientCVar(actorComponent.PlayerSession.Channel, WhiteCVars.LogInChat))
            {
                var logLines = new List<string>();
